...
class Topic(models.Model):
    ...
    def save(self, *args, **kwargs):
        print(self.unit_part.unit.subject.id)
        t=str(self.topic_name + "-" + str(self.unit_part.unit.subject.id))
        self.slug = slugify(t)
...
#     count=models.IntegerField(default=0,null=True,blank=True)
#     def __str__(self):
#         return self.unit.unit_name + " - "+self.user.name
...
#     count=models.IntegerField(default=0,null=True,blank=True)
#     def __str__(self):
#         return self.subject.subject_name + " - "+self.user.name
...
class UnitResources(models.Model):
    user=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.CASCADE)
    unit=models.ForeignKey(Unit,on_delete=models.PROTECT,related_name="unit_links")
    title=models.CharField(max_length=500)
    choice=models.CharField(max_length=200,choices=SUBJECT_RESOURCE_CHOICES,default="other")
    description=models.CharField(max_length=1500,null=True,blank=True)
    url=models.CharField(max_length=500,unique=True)
    date=models.DateField(auto_now_add=True)
    endorsed=models.IntegerField(default=0)
    def __str__(self):
        return self.title + " - " + self.unit.unit_name   

class SubjectResources(models.Model):
    user=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.CASCADE)
    subject=models.ForeignKey(Subject,on_delete=models.PROTECT,related_name="subject_links")
    title=models.CharField(max_length=500)
    choice=models.CharField(max_length=200,choices=SUBJECT_RESOURCE_CHOICES,default="other")
    description=models.CharField(max_length=1500,null=True,blank=True)
    url=models.CharField(max_length=500,unique=True)
    ...
    endorsed=models.IntegerField(default=0)
    def __str__(self):
        return self.title + " - " + self.subject.subject_name  
